...
Never skipped, like the Cloud Run tests in ``test_gcp_cloud_run.py``: run them
only with GCP credentials (``tox -e gcp``). ``GOOGLE_CLOUD_API_KEY`` must
...
``GCP_REGION``; the GPU test uses ``region="auto"``, since L4 capacity varies
by region).
"""
...
def _gpu_provider(provider, region):
    return GCPBatchProvider(
...
def test_gcp_batch_gpu_region_auto(provider):
    """``region="auto"`` finds a region with L4 capacity and runs the job there,
    with the L4 actually visible to the container."""
    assert "L4" in gpu_name(compute=_gpu_spec(_gpu_provider(provider, "auto")))
